...
enum class PlaneType : uint8_t { Left, Right, Bottom, Top, Near, Far };
enum class Halfspace : uint8_t { Inside, Outside, On };
...
inline glm::vec3 intersect3Planes(const Plane& plane1, const Plane& plane2,
  ...
  const glm::mat3 matrix =
      glm::transpose(glm::mat3(plane1.normal, plane2.normal, plane3.normal));
  const glm::vec3 distances(-plane1.distance, -plane2.distance,
                            -plane3.distance);
